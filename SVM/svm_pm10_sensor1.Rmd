...
# Support vector machine regression model for sensor 1, PM10
...
d.mrakova.sensor1.selectedPM10<- d.mrakova.sensor1.small %>% select(T, RH, PM10, p, ambient_light, CO2, VOC_index, created_at)

#standardizacija
PM10_1_standardized <- 
  d.mrakova.sensor1.selectedPM10 %>% 
...
# PM10
validation_index <- createDataPartition(PM10_1_standardized$PM10, p=0.60, list=FALSE)
validation <- PM10_1_standardized[-validation_index,]
d.mrakova.sensor1.selectedPM10 <- PM10_1_standardized[validation_index,]

#dim(d.mrakova.sensor1.selectedPM10)

#levels(d.mrakova.sensor1.selectedPM10$PM10)

summary(d.mrakova.sensor1.selectedPM10)
...
#Scatter Plot
#plot(d.mrakova.sensor1.selectedPM10)

# regression training model SVM
model <- svm(PM10~T + RH + CO2 + p + ambient_light+ VOC_index, data = d.mrakova.sensor1.selectedPM10)
summary(model)

#Predict using SVM regression
pred_PM10_s1 = predict(model, newdata=validation)

validation_new1_s1 <- cbind(validation, pred_PM10_s1)
...
validation_new1_s1_graph <- validation_new1_s1 %>%
   mutate(date = as.POSIXct(created_at), format="%Y-%m-%d %H:%M:%S") %>% select(date, pred_PM10_s1, PM10) %>%
...
# RMSE
sqrt(mean((validation_new1_s1$PM10 - validation_new1_s1$pred_PM10_s1)^2))
...
# R2
cor(validation_new1_s1$PM10, validation_new1_s1$pred_PM10_s1) ^ 2
