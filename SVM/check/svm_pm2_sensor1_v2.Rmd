---
title: "SVR CO2 index sensor 1 10 int"
...
# Support vector machine regression model for sensor 1, CO2

Splitting data into sensors, reducing dataset to keep every 30th observation, standardization and splitting into training and testing data set.
...
d.mrakova.sensor1.selectedCO2<- d.mrakova.sensor1.small %>% select(T, RH, PM10, p, ambient_light, CO2, VOC_index, created_at)
...
# CO2
validation_index <- createDataPartition(VOC_1_standardized$CO2, p=0.60, list=FALSE)
validation <- CO2_1_standardized[-validation_index,]
d.mrakova.sensor1.selectedVOC <- CO2_1_standardized[validation_index,]

#dim(d.mrakova.sensor1.selectedCO2)

#levels(d.mrakova.sensor1.selectedCO2$CO2)

summary(d.mrakova.sensor1.selectedCO2)
...
model <- svm(CO2~T + RH + PM10 + p + ambient_light+ VOC_index, data = d.mrakova.sensor1.selectedCO2)
summary(model)

#Predict using SVM regression
pred_CO2_s1 = predict(model, newdata=validation)

validation_new1_s1 <- cbind(validation, pred_CO2_s1)
...
validation_new1_s1_graph <- validation_new1_s1 %>%
   mutate(date = as.POSIXct(created_at), format="%Y-%m-%d %H:%M:%S") %>% select(date, pred_CO2_s1, CO2) %>%
...
# RMSE
sqrt(mean((validation_new1_s1$CO2 - validation_new1_s1$pred_CO2_s1)^2))
...
# R2
cor(validation_new1_s1$CO2, validation_new1_s1$pred_CO2_s1) ^ 2
