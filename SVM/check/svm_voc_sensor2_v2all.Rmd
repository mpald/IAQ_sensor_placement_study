---
title: "SVR VOC index sensor 2 - 10th"
...
nrow(d.mrakova.main.predict)
head(d.mrakova.main.predict)
#write.csv(d.mrakova.main,'sensor_data_2020.csv')
...
Keeping only needed variables and creating a variable to handle time variable. Removing two extreme outliers (pressure variable) immediately.
...
Removing outliers. Shown bellow, the number of observations with outliers (1st value) and without (2nd value).
...
#complete dataset
outliers.no_total <- d.mrakova.sensor2
outliers.no2 <- d.mrakova.sensor2
...
outliers.no2 <- outliers.no2 %>% filter(VOC_index >= low1 & VOC_index <= up1)
...
outliers.no2 <- outliers.no2 %>% filter(CO2 >= low2 & CO2 <= up2)
...
outliers.no2 <- outliers.no2 %>% filter(p >= low3 & p <= up3)
...
outliers.no2 <- outliers.no2 %>% filter(RH >= low4 & RH <= up4)
...
outliers.no2 <- outliers.no2 %>% filter(PM10 >= low5 & PM10 <= up5)
...
outliers.no2 <- outliers.no2 %>% filter(ambient_light >= low6 & ambient_light <= up6)
nrow(outliers.no2)

d.mrakova.non.dup.predict <- outliers.no2

summary(d.mrakova.non.dup.predict)

# combine both sensors again
#d.mrakova.non.dup.predict <- rbind(d.mrakova.non.dup.predict.1, d.mrakova.non.dup.predict.2)
#nrow(d.mrakova.dup.predict)
```
...
# Support vector machine regression model for sensor 2, VOC index
...
#d.mrakova.sensor1 <- X[[1]]
d.mrakova.sensor2 <- d.mrakova.svm.log

#reducing dataset 
d.mrakova.sensor2.small = d.mrakova.sensor2[seq(1, nrow(d.mrakova.sensor2), 10), ]
nrow(d.mrakova.sensor2.small)

d.mrakova.sensor2.selectedVOC<- d.mrakova.sensor2.small %>% select(T, RH, PM10, p, ambient_light, CO2, VOC_index, created_at)

#standardizacija
VOC_2_standardized <- 
  d.mrakova.sensor2.selectedVOC %>% 
...
validation_index2 <- createDataPartition(VOC_2_standardized$VOC_index, p=0.60, list=FALSE)
validation2 <- VOC_2_standardized[-validation_index2,]
d.mrakova.sensor2.selectedVOC <- VOC_2_standardized[validation_index2,]

#dim(d.mrakova.sensor2.selectedVOC)

#levels(d.mrakova.sensor2.selectedVOC$VOC_index)

summary(d.mrakova.sensor2.selectedVOC)
...
#Scatter Plot
#plot(d.mrakova.sensor2.selectedVOC)

# regression training model SVM
model2 <- svm(VOC_index~T + RH + PM10 + p + ambient_light+ CO2, data = d.mrakova.sensor2.selectedVOC)
summary(model2)

#Predict using SVM regression
pred_VOC_s2 = predict(model2, newdata=validation2)

validation_s2_new1 <- cbind(validation2, pred_VOC_s2)
...
validation_s2_new1_graph <- validation_s2_new1 %>%
   mutate(date = as.POSIXct(created_at), format="%Y-%m-%d %H:%M:%S") %>% select(date, pred_VOC_s2, VOC_index) %>%
  gather(key = "variable", value = "value", -date)

#as.POSIXlt(created_at)
#validation_new1_s1_graph

ggplot(validation_s2_new1_graph, aes(x = date, y = value)) + 
...
# RMSE
sqrt(mean((validation_s2_new1$VOC_index - validation_s2_new1$pred_VOC_s2)^2))
...
# R2
cor(validation_s2_new1$VOC_index, validation_s2_new1$pred_VOC_s2) ^ 2
