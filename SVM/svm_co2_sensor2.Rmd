...
# Support vector machine regression model for sensor 2, CO2
...
d.mrakova.sensor2.selectedCO2<- d.mrakova.sensor2.small %>% select(T, RH, PM10, p, ambient_light, CO2, VOC_index, created_at)

#standardizacija
CO2_2_standardized <- 
  d.mrakova.sensor2.selectedCO2 %>% 
...
validation_index2 <- createDataPartition(CO2_2_standardized$CO2, p=0.60, list=FALSE)
validation2 <- CO2_2_standardized[-validation_index2,]
d.mrakova.sensor2.selectedCO2 <- CO2_2_standardized[validation_index2,]

#dim(d.mrakova.sensor2.selectedCO2)

#levels(d.mrakova.sensor2.selectedCO2$CO2)

summary(d.mrakova.sensor2.selectedCO2)
...
#Scatter Plot
#plot(d.mrakova.sensor1.selectedVOC)

# regression training model SVM
model2 <- svm(CO2~T + RH + PM10 + p + ambient_light+ VOC_index, data = d.mrakova.sensor2.selectedCO2)
summary(model2)

#Predict using SVM regression
pred_CO2_s2 = predict(model2, newdata=validation2)

validation_s2_new1 <- cbind(validation2, pred_CO2_s2)
...
validation_s2_new1_graph <- validation_s2_new1 %>%
   mutate(date = as.POSIXct(created_at), format="%Y-%m-%d %H:%M:%S") %>% select(date, pred_CO2_s2, CO2) %>%
...
# RMSE
sqrt(mean((validation_s2_new1$CO2 - validation_s2_new1$pred_CO2_s2)^2))
...
# R2
cor(validation_s2_new1$CO2, validation_s2_new1$pred_CO2_s2) ^ 2
